...
static PyObject *device_get_color_control_capabilities(PyObject *self, PyObject *args) {
  ...
  k4a_result_t result = k4a_device_get_color_control_capabilities(
      *device_handle, command, &supports_auto, &min_value, &max_value, &step_value, &default_value, &default_mode);
  _gil_restore(thread_state);
  ...
  return Py_BuildValue("I{s:I,s:N,s:i,s:i,s:i,s:i,s:I}", result, "color_control_command", command, "supports_auto",
                       supports_auto ? Py_True : Py_False, "min_value", min_value, "max_value", max_value, "step_value",
                       step_value, "default_value", default_value, "default_mode", default_mode);
}
...
static PyObject *device_start_cameras(PyObject *self, PyObject *args) {
  ...
  PyArg_ParseTuple(args, "OpIIIIpiIIp", &capsule, &thread_safe, &config.color_format, &config.color_resolution,
                   &config.depth_mode, &config.camera_fps, &config.synchronized_images_only,
...
static PyObject *calibration_get_from_raw(PyObject *self, PyObject *args) {
  ...
  thread_state = _gil_release(thread_safe);
  result = k4a_calibration_get_from_raw(raw_calibration, raw_calibration_size, depth_mode, color_resolution,
                                        calibration_handle);
  if (result == K4A_RESULT_FAILED) {
    _gil_restore(thread_state);
    return Py_BuildValue("IN", result, Py_None);
  ...
  PyObject *calibration_capsule =
      PyCapsule_New(calibration_handle, CAPSULE_CALIBRATION_NAME, capsule_cleanup_calibration);
...
static PyObject *transformation_create(PyObject *self, PyObject *args) {
  ...
  *transformation_handle = k4a_transformation_create(calibration_handle);
  ...
  PyObject *transformation_capsule =
      PyCapsule_New(transformation_handle, CAPSULE_TRANSFORMATION_NAME, capsule_cleanup_transformation);
...
static PyObject *transformation_depth_image_to_color_camera(PyObject *self, PyObject *args) {
  ...
  if (K4A_RESULT_SUCCEEDED == res) {
    res = k4a_image_create(k4a_image_get_format(depth_image), RESOLUTION_TO_DIMS[color_resolution][0],
                           RESOLUTION_TO_DIMS[color_resolution][1],
                           RESOLUTION_TO_DIMS[color_resolution][0] * (int)sizeof(uint16_t), depth_image_transformed);
  }

  if (K4A_RESULT_SUCCEEDED == res) {
    res = k4a_transformation_depth_image_to_color_camera(*transformation_handle, depth_image, *depth_image_transformed);
...
static PyObject *transformation_depth_image_to_color_camera_custom(PyObject *self, PyObject *args) {
  ...
  PyArg_ParseTuple(args, "OpO!O!Ip", &capsule, &thread_safe, &PyArray_Type, &d_array, &PyArray_Type, &c_array,
                   &color_resolution, &interp_nearest);
  ...
  if (K4A_RESULT_SUCCEEDED == res) {
    res = k4a_transformation_depth_image_to_color_camera_custom(*transformation_handle, depth_image, custom_image,
                                                                *depth_image_transformed, *custom_image_transformed,
...
static PyObject *transformation_depth_image_to_point_cloud(PyObject *self, PyObject *args) {
  ...
  thread_state = _gil_release(thread_safe);
  if (K4A_RESULT_SUCCEEDED == res) {
    res = k4a_image_create(K4A_IMAGE_FORMAT_CUSTOM, k4a_image_get_width_pixels(depth_image),
                           k4a_image_get_height_pixels(depth_image),
                           k4a_image_get_width_pixels(depth_image) * 3 * (int)sizeof(int16_t), xyz_image);
  }

  if (K4A_RESULT_SUCCEEDED == res) {
    res = k4a_transformation_depth_image_to_point_cloud(*transformation_handle, depth_image, camera, *xyz_image);
    k4a_image_release(depth_image);
  }
  _gil_restore(thread_state);
...
static PyObject *transformation_color_image_to_depth_camera(PyObject *self, PyObject *args) {
  ...
  PyArg_ParseTuple(args, "OpO!O!", &capsule, &thread_safe, &PyArray_Type, &in_depth_array, &PyArray_Type,
                   &in_color_array);
  transformation_handle = (k4a_transformation_t *)PyCapsule_GetPointer(capsule, CAPSULE_TRANSFORMATION_NAME);
  ...
  if (K4A_RESULT_SUCCEEDED == res) {
    ...
    if (K4A_RESULT_SUCCEEDED == res) {
      res = k4a_image_create(K4A_IMAGE_FORMAT_COLOR_BGRA32, k4a_image_get_width_pixels(depth_image),
                             k4a_image_get_height_pixels(depth_image),
                             k4a_image_get_width_pixels(depth_image) * (int)sizeof(uint32_t), transformed_color_image);
    }
  }

  thread_state = _gil_release(thread_safe);
  if (K4A_RESULT_SUCCEEDED == res) {
    res = k4a_transformation_color_image_to_depth_camera(*transformation_handle, depth_image, color_image,
...
static PyObject *calibration_3d_to_3d(PyObject *self, PyObject *args) {
  ...
  res = k4a_calibration_3d_to_3d(calibration_handle, &source_point3d_mm, source_camera, target_camera,
                                 &target_point3d_mm);
  _gil_restore(thread_state);
...
static PyObject *calibration_2d_to_3d(PyObject *self, PyObject *args) {
  ...
  PyArg_ParseTuple(args, "Op(ff)fII", &capsule, &thread_safe, &source_point_x, &source_point_y, &source_depth_mm,
                   &source_camera, &target_camera);
  calibration_handle = (k4a_calibration_t *)PyCapsule_GetPointer(capsule, CAPSULE_CALIBRATION_NAME);
  ...
  res = k4a_calibration_2d_to_3d(calibration_handle, &source_point2d, source_depth_mm, source_camera, target_camera,
  ...
  // Return object...
  return Py_BuildValue("II(fff)", res, valid, target_point3d_mm.xyz.x, target_point3d_mm.xyz.y,
...
static PyObject *playback_get_record_configuration(PyObject *self, PyObject *args) {
  ...
  return Py_BuildValue("I(IIIIiiiiIIII)", result, config.color_format, config.color_resolution, config.depth_mode,
                       config.camera_fps, config.color_track_enabled, config.depth_track_enabled,
                       config.ir_track_enabled, config.imu_track_enabled, config.depth_delay_off_color_usec,
                       config.wired_sync_mode, config.subordinate_delay_off_master_usec,
...
static PyMethodDef Pyk4aMethods[] = {
    ...
    {"capture_get_color_image", capture_get_color_image, METH_VARARGS,
     "Get the color image associated with the given capture"},
    {"capture_get_depth_image", capture_get_depth_image, METH_VARARGS,
    ...
    {"device_get_sync_jack", device_get_sync_jack, METH_VARARGS,
     "Get the device jack status for the synchronization in and "
     "synchronization out connectors."},
    ...
    {"device_get_color_control_capabilities", device_get_color_control_capabilities, METH_VARARGS,
     "Get device color control capabilities."},
    {"device_get_calibration", device_get_calibration, METH_VARARGS, "Get device calibration handle."},
    {"device_get_raw_calibration", device_get_raw_calibration, METH_VARARGS,
     "Get device calibration in text/json format."},
    {"calibration_get_from_raw", calibration_get_from_raw, METH_VARARGS,
     "Create new calibration handle from raw json."},
    {"transformation_create", transformation_create, METH_VARARGS, "Create transformation handle from calibration"},
    {"transformation_depth_image_to_color_camera", transformation_depth_image_to_color_camera, METH_VARARGS,
     "Transforms the depth map into the geometry of the color camera."},
    {"transformation_depth_image_to_color_camera_custom", transformation_depth_image_to_color_camera_custom,
     METH_VARARGS,
     "Transforms the custom & depth map into the geometry of the color "
     "camera."},
    {"transformation_color_image_to_depth_camera", transformation_color_image_to_depth_camera, METH_VARARGS,
    ...
    {"playback_get_calibration", playback_get_calibration, METH_VARARGS,
     "Extract calibration and create handle from recording"},
    {"playback_get_raw_calibration", playback_get_raw_calibration, METH_VARARGS,
     "Extract calibration json from recording"},
    {"playback_seek_timestamp", playback_seek_timestamp, METH_VARARGS, "Seek playback file to specified position"},
    {"playback_get_record_configuration", playback_get_record_configuration, METH_VARARGS,
     "Extract record configuration"},
